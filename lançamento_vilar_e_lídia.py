...
def calcular_tempo_total_voo(velocidade_inicial, angulo, gravidade=10):
    return (2 * velocidade_inicial * math.sin(math.radians(angulo))) / gravidade

def calcular_alcance_horizontal(velocidade_inicial, angulo, gravidade=10):
    return (velocidade_inicial ** 2 * math.sin(2 * math.radians(angulo))) / gravidade

def calcular_velocidades_instantaneas(velocidade_inicial, angulo, tempo, gravidade=10):
...
def main():
    ...
    print(f"2. Tempo Total de Voo: {tempo_total_voo:.2f} seg")
    ...
    print(f"5. Distância Horizontal entre o Ponto de Lançamento e o Local da Queda: {alcance_horizontal:.2f}m")
